...
router.post('/payment', async (req, res) => {
    ...
    try {
        ...
        const newOrder = await createOrder(orderCount, paymentData.items, total);
        await incrementOrderCount();
        
        console.log(`Order (#${orderCount}) successful. Total: $${total}:`);
...
router.get('/order-count', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: "Order count not found." });
    }
});
